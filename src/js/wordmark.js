...
function initLaseretchWordmark(root) {
...
function initCanvasWordmark(root) {
  const canvas = root?.querySelector("[data-wordmark-canvas]");
  const fallback = root?.querySelector("[data-wordmark-fallback]");
  const reducedMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)");
  const printMedia = window.matchMedia?.("print");
...
export function initWordmark() {
  ...
  if (root.dataset.wordmarkEnhancement === EFFECT) {
    initLaseretchWordmark(root);
  } else {
    initCanvasWordmark(root);
  }
}
