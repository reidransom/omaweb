...
<section class="prose" aria-labelledby="community-start-title">
  ...
  {% if discord %}
  <p class="button-group"><a class="button" href="{{ discord.url | escape }}" rel="noreferrer">Join the Discord</a></p>
  {% endif %}
</section>
...
<section class="prose" aria-labelledby="community-places-title">
  ...
  <ul>
    ...
    {% if plugins %}<li><a href="{{ plugins.url | escape }}" rel="noreferrer">Plugins</a> is the authoritative catalog for extensions made around Omarchy.</li>{% endif %}
...
<section class="prose" aria-labelledby="community-contribute-title">
  ...
  <ul>
    {% if code %}<li>For code, start at <a href="{{ code.url | escape }}" rel="noreferrer">the Omarchy repository</a>.</li>{% endif %}
    {% if plugins %}<li>For an extension, publish it through <a href="{{ plugins.url | escape }}" rel="noreferrer">Plugins</a>.</li>{% endif %}
    <li>For community stewardship, visit the <a href="{{ '/teams/rangers/' | relative_url }}">Rangers</a>.</li>
    {% if discord %}<li>For a question, an idea, or a photo you want considered for the gallery, bring it to <a href="{{ discord.url | escape }}" rel="noreferrer">Discord</a>.</li>{% endif %}
  </ul>
</section>
